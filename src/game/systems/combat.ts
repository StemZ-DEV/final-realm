...
export interface EnemyInstance {
  ...
  status: ActiveStatus[];
  lootTableId: string;
  variant: "normal" | "elite" | "boss" | "ambush";
}

export function computePlayerStats(player: Player, registry: ContentRegistry): DerivedStats {
  const bonus: Stats = {
    strength: 0,
    defense: 0,
    agility: 0,
  ...
  for (const entry of equipment) {
    ...
    if (entry.enhancements) {
      for (const enh of entry.enhancements) {
        const key = enh.stat as keyof Stats;
        bonus[key] += enh.value;
      }
    }
  }
...
export function createEnemyInstance(
  ...
  playerLevel: number,
  variant: EnemyInstance["variant"],
  difficulty: { enemyHpScale: number; enemyAtkScale: number }
): EnemyInstance {
  ...
  if (variant === "elite") hp = Math.round(hp * 1.4);
  if (variant === "boss") hp = Math.round(hp * 2);
...
export function attackDamage(attacker: DerivedStats, defender: DerivedStats): number {
  const variance = randInt(-2, 2);
  const raw = attacker.attack - defender.defense + variance;
  return Math.max(1, raw);
}

export function applyDamage(target: { hp: number }, amount: number): number {
  const dmg = Math.max(0, Math.floor(amount));
  target.hp = Math.max(0, target.hp - dmg);
  return dmg;
}

export function applyHeal(target: { hp: number; maxHp: number }, amount: number): number {
...
export function tickStatuses(
  target: { hp: number; maxHp: number; status: ActiveStatus[] },
  ...
  for (const status of target.status) {
    ...
    status.remaining -= 1;
    if (status.remaining > 0) remaining.push(status);
    else log(`${def.name} fades.`);
  }

  target.status = remaining;
}

export function applyStatus(target: { status: ActiveStatus[] }, statusId: string, duration: number): void {
...
export function consumeItem(
  ...
  if (!def) return "";
  if (!def.effects || def.effects.length === 0) return "Nothing happens.";
...
function weightedLoot(entries: Array<{ id: string; weight: number; min: number; max: number }>) {
...
function normalizeStats(partial: Partial<Stats>): Stats {
  return {
...
function applyStats(target: Stats, extra: Partial<Stats>): void {
