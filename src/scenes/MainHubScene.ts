...
export class MainHubScene implements Scene<GameContext> {
  ...
  handleInput(ctx: GameContext, action: InputAction): void {
    ...
    if (action.type === "confirm") {
      ...
      if (choice === "town") ctx.scenes.push(new TownScene(), ctx);
      if (choice === "wild") ctx.scenes.push(new WildernessScene(), ctx);
      if (choice === "inventory") ctx.scenes.push(new InventoryScene(), ctx);
      if (choice === "stats") ctx.scenes.push(new StatsScene(), ctx);
      if (choice === "settings") ctx.scenes.push(new SettingsScene(), ctx);
      if (choice === "history") ctx.scenes.push(new RunHistoryScene(), ctx);
      if (choice === "savequit") {
  ...
  render(ctx: GameContext, frame: Frame): void {
    ...
    const header = renderHeader(
      "Town Square",
      `Welcome, ${ctx.game.currentSave.player.name}`,
      Math.min(frame.width - 4, 60)
    );
    const pos = centerPosition(frame.width, frame.height, header);
    frame.blit(pos.x, 1, header);
