...
import { applyDamage, attackDamage, computePlayerStats, createEnemyInstance, consumeItem, grantLoot, tickStatuses, } from "../game/systems/combat.js";
...
import { nowIso } from "../game/utils.js";
export class CombatScene {
    ...
    enemy = null;
    playerGuard = false;
    constructor(zone, enemyId, variant) {
        this.zone = zone;
    ...
    handleInput(ctx, action) {
        if (!ctx.game.currentSave || !this.enemy)
            return;
        ...
        if (action.type === "confirm") {
            ...
            if (choice === "item") {
                ctx.scenes.push(new CombatItemScene((index) => {
    ...
    handleDefeat(ctx) {
        ...
        ctx.scenes.pop(ctx);
        ctx.scenes.push(new MessageScene(`You lost the fight... You scurried to the pharmacy with 1 HP remaining. (-${loss}g)`), ctx);
    }
    pushLog(message) {
        this.log.push(message);
    }
}
function computeEnemyStats(enemy) {
